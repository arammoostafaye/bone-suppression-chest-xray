# Derived from app.py (the exact mirror of the HF Space) so the demo also runs
# where there is no GPU - e.g. GitHub Codespaces, which offers CPU-only machines
# on personal accounts. app.py is left byte-identical to the upstream Space.
# Diff vs app.py: device chosen at runtime (cuda if available, else cpu) and the
# server binds 0.0.0.0 so Codespaces can forward the port.
...
# Device is chosen at runtime: CUDA on the HF ZeroGPU Space, CPU elsewhere (e.g. GitHub Codespaces).
...
try:
    ...
    print(f"[startup] module-scope .to({DEVICE!r}) applied to both traces", flush=True)
...
def _ensure_device():
    """Move both traces onto the target device if they are not already there.
    ...
    for model in (bone_model, lung_model):
        try:
            ready = next(model.parameters()).device.type == DEVICE
        except StopIteration:
            ready = True  # parameterless trace: nothing to move
        if not ready:
            model.to(DEVICE)
...
def decompose(
    ...
    with torch.no_grad():
        ...
        xs = torch.from_numpy(soft.astype(np.float32))[None, None].to(DEVICE)
