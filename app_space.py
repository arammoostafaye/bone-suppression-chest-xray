# Identical to app.py (the exact mirror of the upstream HF Space) except for
# the PWA lines in launch(): favicon_path + pwa=True. Everything else - the
# @spaces.GPU decorator, the hard-coded CUDA path, the preprocessing - is
# untouched, so this is the file a personal HF Space should run.
...
# Icon for the installable PWA (see launch() below). Kept next to the desktop
# build so one asset serves both.
_ICON = os.path.join(os.path.dirname(os.path.abspath(__file__)), "desktop", "icon.png")
